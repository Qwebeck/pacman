    ...
    def update(self):
        if self.game.player.keys != -1:
            ...
            if pg.sprite.spritecollide(self,self.game.player_group,False) and self.game.is_pellet == True:
                ...
                self.picked_power = 200 * self.game.ghost_counter % 4   # 1 - fruit
                self.picked_power_pos = (self.game.player.rect.centerx // self.game.tilesize , self.game.player.rect.centery // self.game.tilesize)
...
class Blinky(Ghost):
    ...
    def behaviour(self):
        ...
        if vec(self.player_now) != vec(-5,-int((self.game.GRIDWIDTH-map_len)/2)):
            ...
            if self.game.scatter_mode == True and self.game.p_ch_index < 2 and self.path[-1] != (1 ,2):
                # self.path.pop(0)
...
#         if self.game.is_pellet == True:
#             self.is_ghost_pellet = True
#             self.game.pellet_index += 1
#         if self.game.pellet_index % 4 == 0:
...
#         #define following function to catch the player
#         #chanching scatter state
#         if self.game.session_time > 3 and  self.game.scatter_mode == True and (self.path == 0 or (self.path == 0 or len(self.path) <= 2)):
#             self.game.scatter_mode = False
#             self.path = breadth_search(self.game.maze,self.ghost_now,self.player_now)

#         elif ( self.path == 0 or (self.path == 0 or len(self.path) <= 2) ) and self.game.scatter_mode != True:
#             self.path = breadth_search(self.game.maze,self.ghost_now,self.player_now)  

#         if len(self.path) <= 1 and self.is_ghost_pellet == False and self.game.session_time < 20:
#             return 0

#         if len(self.path) <= 1 and self.is_ghost_pellet == False:
#             self.game_over()
#             return 0

#         elif self.is_ghost_pellet == True and len(self.path) <= 1:
#             return 0 
...
#         # try:
#         #     print(ghost_now)
#         #     maze[ghost_now[0]][ghost_now[1]]
#         #     print("Prevent:",maze[ghost_now[0]][ghost_now[1]])
#         # except IndexError:
#         #     print("called bp")
#         #     self.path = breadth_search(maze, ghost_now,ghost_house,emergency=True)
#         #     print(self.path)
...
#             # if self.is_ghost_pellet:
#             # self.switching_pellet()
          
#             if pg.sprite.spritecollide(self,self.game.player_group,False) and self.is_ghost_pellet == True:
#                 # print("Eated: ",self.is_ghost_pellet)
...
#             elif pg.sprite.spritecollide(self,self.game.player_group,False) and self.is_ghost_pellet == False and self.eated == False:
#                 self.game_over()
